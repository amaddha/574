...
def testMVImageClassifier(dataset_path, model, info, pooling = 'mean', cuda=False, verbose=False):
    ...
    for idx, c in enumerate(info['category_names']):
        ...
        for s in shape_dirs:
            ...
            # Calculating shape-level predictions based on pooling strategy
            if pooling == 'mean':
                shape_scores = np.mean(scores, axis=0)
            elif pooling == 'max':
                shape_scores = np.max(scores, axis=0)
            else:
                raise ValueError("Pooling method must be 'mean' or 'max'.")

            predicted_label = np.argmax(shape_scores)
