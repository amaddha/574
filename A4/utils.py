...
class SdfDataset(data.Dataset):
    def __init__(self, points=None, normals=None, phase='train', args=None):
        ...
        if self.phase == 'test':
            ...
            if phase == 'val':
                ...
                sample_indices = torch.randint(0, self.points.shape[0], (self.number_samples,))
                
                # self.samples_xyz = self.p_i + np.random.random(size=(self.points.shape[0], 3))
                self.samples_sdf = np.random.randn(self.number_samples, 1) * self.sample_std
                self.samples_xyz = self.points[sample_indices] + (self.samples_sdf * self.normals[sample_indices])
                # ***********************************************************************
    ...
    def __getitem__(self, idx):
        ...
        if self.phase == 'val':
            ...
            sample_indices = torch.randint(0, self.points.shape[0], (this_bs,))
            gt_sdf = np.random.randn(this_bs, 1) * self.sample_std

            # xyz = self.points + np.random.random(size=(self.points.shape[0], 3))
            xyz = self.points[sample_indices] + (gt_sdf * self.normals[sample_indices])
            # ***********************************************************************
